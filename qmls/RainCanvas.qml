...
Canvas {
    ...
    function createLine(e) {
        ...
        var line = {
            ...
            : temp,
            color// 雨滴的颜色
            : getRgb(Math.floor(temp * 255 / 75), Math.floor(temp * 255 / 75),
    ...
    function createDrop(x, y) {
        ...
        var drop = {
            ...
            vx// vx 表示 x轴的值 变化的速度
            : (Math.random() - 0.5) * 8,
            vy// vy 表示 y轴的值 变化的速度 取值范围：-3 到 -9
            : Math.random() * (-6) - 3,
            radius// 圆弧的半径
            : Math.random() * 1.5 + 1
        }
    ...
    function render() {
        ...
        dropList.forEach(function (e) {
            ctx.beginPath()
            ctx.arc(e.posx, e.posy, e.radius, Math.random() * Math.PI * 2,
                    1 * Math.PI)
            ctx.stroke()
        })
    ...
    onPaint: {
        ...
        var endLine = canvas.height - Math.random() * canvas.height / 5
